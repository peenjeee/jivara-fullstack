...
describe("patient detail helpers", () => {
  ...
  it("counts activity distribution and patient summary", () => {
    ...
    expect(getPatientSummary({ patient, schedules: [
      schedule,
      { ...schedule, id: "SCH-002", reminderEnabled: true, status: "Selesai" },
      { ...schedule, id: "SCH-003", reminderEnabled: true, status: "Nonaktif" },
    ], activities: [activity], activityDistribution: getActivityDistribution([activity]), scans: [] })).toMatchObject({
