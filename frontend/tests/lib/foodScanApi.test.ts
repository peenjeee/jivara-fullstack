...
describe("foodScanApi", () => {
  ...
  it("uploads, detects, checks interactions, and maps analysis", async () => {
    ...
    expect(mockedPost).toHaveBeenNthCalledWith(4, "/nutrition-estimates", { scanId: "img-1", detectedItems: [{ label: "milk", confidence: 0.95 }] });
    expect(analysis).toMatchObject({
      overallRisk: "High Risk",
      scan: { id: "img-1", patientId: "patient-1", foodName: "Susu", risk: "High Risk", aiReasoning: "Model Jivara Food Detection mendeteksi Susu dalam 0,1 detik dan mencocokkannya dengan obat aktif pasien.", detectedItems: [{ label: "milk", labelDisplay: "Susu", confidence: 0.95, boundingBox: { x1: 10, y1: 20, x2: 110, y2: 220 } }] },
      ...
      nutritionItems: [expect.objectContaining({ foodDisplay: "Susu", source: "Jivara Nutrition", nutrition: { calories: 60, proteinG: 3.2, fatG: 3.3, carbsG: 4.8 } })],
  ...
  it("only maps high severity to high risk without marking medium pairs as high", async () => {
    mockedGet.mockResolvedValueOnce({ data: { data: { id: "patient-1", fullName: "Budi Santoso" } } });
    mockedPost
      .mockResolvedValueOnce({ data: { data: { image_id: "img-1", upload_url: "/uploads/img-1.jpg", timestamp: "2026-05-09T08:00:00.000Z" } } })
      ...
      ], patient_medications: ["CELESTAR", "SIMVASTATIN"], analyzed_medications_count: 2, recommended_foods: [], foods_to_avoid: [], overall_risk_level: "sedang", overall_recommendation: "Ditemukan potensi interaksi obat-makanan. Baca penjelasan AI per obat dan konsultasikan dengan dokter atau apoteker.", disclaimer: "Konsultasikan ke tenaga kesehatan." } } })
      .mockResolvedValueOnce({ data: { data: { items: [], total: { calories: 0, protein_g: 0, fat_g: 0, carbs_g: 0 } } } });
    ...
    expect(analysis.scan.recommendation).toBe("Ditemukan potensi interaksi obat-makanan. Baca penjelasan AI per obat dan konsultasikan dengan dokter atau apoteker.");
    expect(analysis.interactions.map((interaction) => interaction.risk)).toEqual(["Low Risk", "Low Risk"]);
  });
  ...
  it("maps analyzed medication count and recommendations from scan detail API", async () => {
    ...
    expect(analysis.scan.aiReasoning).toBe("Model Jivara Food Detection menganalisis Kangkung dalam 0,1 detik dan mencocokkannya dengan obat aktif pasien.");
  ...
  it("keeps medium scan detail interactions as low-risk UI cards", async () => {
    mockedGet.mockResolvedValueOnce({
      data: {
        data: {
          id: "scan-medium",
          patientId: "patient-1",
          imageUrl: "/uploads/scan-medium.jpg",
          overallRiskLevel: "sedang",
          overallRecommendation: "Ada interaksi sedang, baca penjelasan AI per obat.",
          modelVersion: "v1",
          inferenceTimeMs: 120,
          createdAt: "2026-05-09T08:00:00.000Z",
          detectedItems: [{ id: "item-1", label: "nasi-goreng", labelDisplay: "Nasi Goreng", confidence: 0.9 }],
          interactions: [{
            id: "interaction-1",
            foodItem: "nasi-goreng",
            medication: "CELESTAR",
            severity: "sedang",
            interactionDescription: "Penjelasan AI untuk kombinasi sedang.",
            recommendation: "",
          }],
          patientMedications: ["CELESTAR"],
          analyzedMedicationCount: 1,
          recommendedFoods: [],
          foodsToAvoid: [],
        },
    ...
    const analysis = await getFoodScanAnalysisFromApi("scan-medium");

    expect(analysis.overallRisk).toBe("Low Risk");
    expect(analysis.scan.risk).toBe("Low Risk");
    expect(analysis.scan.result).toBe("Ditemukan potensi interaksi obat-makanan.");
    expect(analysis.scan.recommendation).toBe("Ada interaksi sedang, baca penjelasan AI per obat.");
    expect(analysis.interactions).toEqual([
      expect.objectContaining({
        foodItem: "nasi-goreng",
        risk: "Low Risk",
        reasoning: "Penjelasan AI untuk kombinasi sedang.",
      }),
    ]);
  });
