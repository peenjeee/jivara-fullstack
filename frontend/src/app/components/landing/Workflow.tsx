    ...
    <section id="alur" className="py-20 md:pt-[128px] md:pb-[112px] px-5 md:px-[58px] bg-bg" aria-labelledby="projects-title">
      <div className="mb-8 md:mb-12">
        <h2 id="projects-title" className="font-display text-[clamp(28px,8vw,42px)] lg:text-[72px] font-extrabold leading-none lg:leading-[0.9]">
          <span className="block text-text-main">Alur</span>
          <span className="block text-text-main">Sistem</span>
        </h2>
      </div>
      <div className="grid max-w-[1280px] mx-auto grid-cols-1 md:grid-cols-2 lg:grid-cols-12 items-stretch gap-6 md:gap-[30px]">
        {steps.map((step) => (
          <article 
            key={step.number}
            className={`relative min-h-[280px] md:min-h-[320px] flex flex-col lg:flex-row items-start lg:items-end overflow-hidden rounded-[36px] bg-primary text-white border border-white/10 isolate transform-gpu transition-all duration-[350ms] ease-[cubic-bezier(0.16,1,0.3,1)] hover:-translate-y-[10px] hover:rotate-[-0.4deg] hover:shadow-[0_28px_70px_var(--line)] col-span-1 md:col-span-1 ${step.span} p-10 lg:p-0`}
          >
            {step.label && (
              <span className="hidden lg:block absolute top-[44px] left-[44px] text-text-main/5 font-display text-lg">{step.label}</span>
            )}
            <span className="relative lg:absolute lg:top-8 lg:right-8 w-12 lg:w-[58px] h-12 lg:h-[58px] grid place-items-center rounded-full bg-white text-primary text-lg lg:text-[22px] font-black mb-5 lg:mb-0">
              {step.number}
            </span>
