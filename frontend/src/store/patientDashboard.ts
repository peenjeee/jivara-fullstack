...
interface PatientDashboardState {
  ...
  readonly setScheduleContext: (context: { readonly patientId: string | null; readonly confirmedScheduleDates: Readonly<Record<string, readonly string[]>> }) => void;
...
export const usePatientDashboardStore = create<PatientDashboardState>()((set) => ({
  ...
  setScheduleContext: ({ patientId, confirmedScheduleDates }) => set({ patientId, confirmedScheduleDates }),
