...
export default function ActivityFeed({ activities, visibleCount, onLoadMore, onMarkRead, onViewDetail }: ActivityFeedProps) {
        ...
        <div key={group.dateKey} className="relative">
          ...
          <div className="space-y-4 border-l-2 border-line pl-4 sm:pl-6">
            {group.items.map((activity, index) => (
              <ActivityFeedItem key={activity.id} activity={activity} index={index} onMarkRead={onMarkRead} onViewDetail={onViewDetail} />
            ))}
          </div>
        </div>
      ))}
