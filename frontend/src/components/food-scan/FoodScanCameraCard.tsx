...
interface FoodScanCameraCardProps {
  ...
  readonly cameraStatus: "disabled" | "starting" | "error" | "ready";
...
export default function FoodScanCameraCard({ cameraDevices, cameraError, cameraAspectRatio, cameraKey, cameraStatus, isScanning, onCameraError, onCameraReady, onRetryCamera, onScan, onSelectCamera, onUploadImage, selectedCameraId, webcamRef }: FoodScanCameraCardProps) {
  const shouldAnimate = useDashboardEntranceMotion();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const isCameraEnabled = cameraStatus === "ready" || cameraStatus === "starting";
  ...
  return (
    <m.section className="min-w-0 overflow-hidden rounded-[32px] bg-white p-4 shadow-[0_10px_30px_rgba(15,23,42,0.08)] sm:p-8" {...getDashboardEntranceMotion(shouldAnimate, 0.08, 22)}>
      ...
      <div data-food-scan-camera className="relative mx-auto flex w-full max-w-[520px] min-w-0 items-center justify-center overflow-hidden rounded-[28px] border-2 border-dashed border-primary/45 bg-surface" style={{ aspectRatio: cameraAspectRatio ? String(cameraAspectRatio) : "1 / 1" }}>
        {isCameraEnabled && !cameraError && (
          <Webcam key={cameraKey} ref={webcamRef} audio={false} className="size-full object-cover" disablePictureInPicture forceScreenshotSourceSize imageSmoothing muted screenshotFormat="image/jpeg" screenshotQuality={0.92} videoConstraints={getVideoConstraints(selectedCameraId)} onUserMedia={onCameraReady} onUserMediaError={onCameraError} />
        )}

        {isCameraStarting && !isScanning && <div className="absolute inset-[2px] rounded-[26px] flex items-center justify-center bg-dark/75 text-white"><Loader2 className="size-12 animate-spin" aria-hidden="true" /></div>}

        {isScanning && <div className="absolute inset-[2px] flex min-w-0 flex-col items-center justify-center gap-4 rounded-[26px] bg-dark/55 px-4 text-center text-white backdrop-blur-sm"><Loader2 className="size-12 shrink-0 animate-spin" /><p className="max-w-full text-xs font-bold uppercase tracking-[0.14em] sm:text-sm">Menganalisis makanan</p></div>}
        ...
        {(!isCameraEnabled || cameraError) && !isCameraStarting && !isScanning && (
          <div className="absolute inset-[2px] flex flex-col items-center justify-center gap-4 rounded-[26px] bg-surface px-6 text-center">
            {cameraError ? <CameraOff className="size-12 text-danger" /> : <Camera className="size-12 text-primary" />}
            <div>
        ...
        {isCameraEnabled && !cameraError && !isCameraStarting && !isCameraReady && <div className="absolute inset-[2px] flex flex-col items-center justify-center gap-4 rounded-[26px] bg-surface px-6 text-center"><Camera className="size-12 text-primary" /><p className="text-sm font-bold uppercase tracking-[0.14em] text-muted">Menunggu kamera</p></div>}
      </div>
